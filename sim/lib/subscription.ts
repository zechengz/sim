...
export async function isProPlan(userId: string): Promise<boolean> {
  try {
    ...
    for (const membership of memberships) {
      ...
      if (orgHasProPlan) {
        logger.info('User has pro plan via organization', { userId, orgId: membership.organizationId })
        return true
      }
    }
    
    // If no org subscriptions, check direct subscriptions
    const directSubscriptions = await db.select()
      .from(schema.subscription)
...
export async function isWorkspaceCollaborationEnabled(userId: string): Promise<boolean> {
  try {
    ...
    if (activeDirectSubscription && activeDirectSubscription.limits?.workspaceCollaborationEnabled) {
      return true
    }
